...
amo_pos_250 <- read_csv("Synthetic_data/Pos_250_years_CONUS.csv", col_names = FALSE)
...
amo_pos_250_clean %>% mutate(date_check = lubridate::ymd_hm(date)) %>% head()

amo_pos_250_clean %>% mutate(date_check = lubridate::ymd_hm(date)) %>% filter(is.na(date_check)) %>% head()

write_csv(amo_pos_250_clean, "amo_pos_250_clean_input_test.csv")

## print test

amo_pos_250_clean %>% head() %>% print()
...
amo_pos_250_clean_get_grid <- parLapply(cl,
                                        amo_pos_250_clean_split,
                                        fun = get_grid_winds)
stopCluster(cl)
end <- proc.time()
print(end - start)

save(amo_pos_250_clean_get_grid, file = "amo_pos_250_clean_get_grid_alt_date_CONUS.rda")
...
amo_pos_250_clean_get_grid_TEST_comb <- do.cpos("rbind", amo_pos_250_clean_get_grid)
...
write_csv(amo_pos_250_clean_get_grid_TEST_comb, "amo_pos_250_clean_get_grid_TEST_comb_1s_SHORT.csv")
