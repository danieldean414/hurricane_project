...
predict.tcExcessMort<-function(modobj=modobj, newdata){
  ...
  X<-model.matrix(~rcspline.eval(vmax_sust,inclx = T,nk=4)+
                    sust_dur+exposure+poverty_prop+white_prop+owner_occupied_prop+
                    age_pct_65_plus_prop+median_age+scale(population_density)+scale(median_house_value)+
                    no_grad_prop+rcspline.eval(scale(year),inclx = T,nk=3)+coastal,data=newdata)
